...
def class_cross_entropy_loss(output, label, ignored_label=None):
    """Define the class balanced cross entropy loss to train the network
    Args:
    output: Output of the network
    label: Ground truth label
    ...
    num_total = torch.ge(labels, 0).float().sum()

    if ignored_label is not None:
        w_void = 1.0 - torch.eq(label, ignored_label).float()
        loss_val = torch.mul(w_void, loss_val)
        # num_total = num_total - torch.eq(label, ignored_label).float().sum()
        num_total = torch.sum(w_void)

    final_loss = -torch.sum(loss_val)/num_total
