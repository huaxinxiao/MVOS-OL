...
# testing save root
name_suffix = 'adapt_{}_{}_{}_{:.2f}_{:.2f}_{}_{}'.format(n_meta_init, n_meta_update, adapt_interval, beta_0, beta_1, pos_pred_thres, neg_dist_thres)
meta_model_name = meta_restore_from.split('/')[-1][:-4]
save_root_prefix = '_'.join(['./results/davis-16-val/' + meta_model_name, name_suffix])
...
for i in range(seq_numbers):
    ...
    for j in range(1, cur_seq_range):
        ...
        cur_img = Variable(load_image_label_davis16(img_file)).cuda()
