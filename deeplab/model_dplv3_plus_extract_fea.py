...
def fun_bottleneck_fea(x, weights, bn_params, layer_name, dilation, residual):
...
def fun_layer4_fea(x, weights, bn_params):
    ...
    out = fun_bottleneck_fea(x, weights, bn_params, layer_name+'0.', dilation[0], downsample)

    for i in range(1, len(dilation)):
        residual = out
        cur_layer_name = layer_name + str(i) + '.'
        out = fun_bottleneck_fea(out, weights, bn_params, cur_layer_name, dilation[i], residual)
...
def fun_layer5_fea(res2_fea, res5_fea, weights, bn_params):
     ...
     encoder_fea_up = F.upsample(input=encoder_fea, size=(res2_fea.size(2), res2_fea.size(3)), mode='bilinear')
     out = torch.cat((encoder_fea_up, low_level_fea), dim=1)
     out = fub_cbr_fea(out, weights, bn_params, 'layer5.decode_1.0', 'layer5.decode_1.1', 1, 1)
     out_2 = fub_cbr_fea(out, weights, bn_params, 'layer5.decode_2.0', 'layer5.decode_2.1', 1, 1)
     out = conv2d(out_2, weights['layer5.final.weight'], weights['layer5.final.bias'], stride=1, padding=0, dilation=1)
     return low_level_fea, encoder_fea, out_2, out
...
class ResNet(nn.Module):
    ...
    def forward(self, x, weights=None, train_mode=None, bn_state=None):
        ...
        if weights == None:
            ...
            if train_mode == 'block_4_5':
                x = fun_layer4_fea(x, weights, bn_state)
                low_level_fea, encoder_fea, out_2, out = fun_layer5_fea(res2_fea, x, weights, bn_state)
            elif train_mode == 'block_5':
                x = self.layer4(x)
                low_level_fea, encoder_fea, out_2, out = fun_layer5_fea(res2_fea, x, weights, bn_state)
            else:
                raise ValueError('Undefined training mode: {}.'.format(train_mode))
        return x, low_level_fea, encoder_fea, out_2, out
